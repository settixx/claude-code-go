...
- 多模型支持与别名快捷切换
- 权限系统 + 安全校验链
- CLAUDE.md 配置：Frontmatter 条件、文件锁、信任系统
...
**开发体验**
- 20+ 斜杠命令（`/commit`、`/review`、`/plan`、`/agents`、`/tasks`……）
- Plan 模式切换用于只读探索
- 上下文压缩 + Token 预算管理
- Bridge 协议 + WebSocket 渲染器，IDE 集成
- 通知系统 + 频率限制
...
├── bridge/             IDE / 无头集成 · WebSocket 渲染器
├── buddy/              虚拟伙伴（精灵 · 心情 · 观察者）
├── cli/                参数解析 · REPL · 斜杠命令（核心 + 开发 + 会话 + agents + tasks + MCP + plan）
├── config/             路径 · CLAUDE.md · Frontmatter · 文件锁 · 信任系统
├── coordinator/        多 Agent：编排器 · Worker Pool · 团队 · Worktree
├── errors/             结构化错误类型
├── interfaces/         核心接口定义
├── mcp/                MCP 客户端 · SSE 传输 · OAuth · Prompts · Bootstrap · Adapter
├── notify/             桌面通知 · 频率限制
...
│   ├── lsp/            语言服务器协议工具
│   └── ...             其余 14 个内置工具
├── tui/                Bubble Tea v2 · 模型 · 输入 · 视口 · 权限 · 伙伴组件 · 费用追踪 · Git 分支 · Markdown · 语法高亮
...
| 语法高亮 | [Chroma v2](https://github.com/alecthomas/chroma) |
| Markdown | [Glamour](https://github.com/charmbracelet/glamour) |
